...
const Sidebar: React.FC<SidebarProps> = ({ isOpenOnMobile, toggleMobileSidebar }) => {
  ...
  const NavLinkItem: React.FC<{ item: NavItem }> = ({ item }) => (
    ...
    >
      <item.icon className="w-5 h-5 mr-3 transition-colors text-slate-400 group-hover:text-rose-600 dark:group-hover:text-rose-500 group-[.active]:text-rose-500 dark:group-[.active]:text-rose-400" />
      {item.name}
    </NavLink>
  );
  ...
  return (
    <div 
      className={`w-64 bg-white dark:bg-slate-900 h-screen flex flex-col fixed inset-y-0 left-0 z-30 border-r border-slate-200 dark:border-slate-800
