    ...
    const handleDelete = async (id: string) => {
        if (window.confirm('Ви впевнені, що хочете видалити цю витрату?')) {
            ...
            try {
                ...
                fetchExpenses(currentPage > newTotalPages ? newTotalPages : currentPage);
            } catch (err: any) {
                setPageError(err.message);
            } finally {
                setIsLoading(false);
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Керування витратами</h2>
                ...
                <div className="overflow-x-auto hidden md:block">
                    <table className="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {isLoading ? (
                                <tr><td colSpan={5} className="text-center py-10 text-slate-500">Завантаження...</td></tr>
                            ) : expenses.length > 0 ? (
                                expenses.map(exp => (
                                ...
                                <tr><td colSpan={5} className="text-center py-10 text-slate-500">Витрат не знайдено.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div role="dialog" aria-modal="true" className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-lg">
                        <div className="flex justify-between items-center pb-4 mb-6 border-b">
                            <h3 className="text-xl font-semibold">{editingExpense ? 'Редагувати' : 'Додати'} витрату</h3>
                            <button onClick={closeModal} disabled={isLoading}><XMarkIcon className="w-6 h-6"/></button>
                        </div>
                        {modalError && <div role="alert" className="mb-4 p-3 bg-red-50 text-red-700 rounded-lg">{modalError}</div>}
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium text-slate-700">Назва</label>
                                <input type="text" name="name" id="name" value={currentExpense.name || ''} onChange={handleInputChange} required className="mt-1 block w-full p-2.5 border-slate-300 rounded-lg" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label htmlFor="amount" className="block text-sm font-medium text-slate-700">Сума (₴)</label>
                                    <input type="number" name="amount" id="amount" value={currentExpense.amount || ''} onChange={handleInputChange} required step="0.01" min="0.01" className="mt-1 block w-full p-2.5 border-slate-300 rounded-lg" />
                                </div>
                                <div>
                                    <label htmlFor="date" className="block text-sm font-medium text-slate-700">Дата</label>
                                    <input type="date" name="date" id="date" value={currentExpense.date || ''} onChange={handleInputChange} required className="mt-1 block w-full p-2.5 border-slate-300 rounded-lg" />
                                </div>
                            </div>
                            <div>
                                <label htmlFor="notes" className="block text-sm font-medium text-slate-700">Нотатки</label>
                                <textarea name="notes" id="notes" value={currentExpense.notes || ''} onChange={handleInputChange} rows={3} className="mt-1 block w-full p-2.5 border-slate-300 rounded-lg" />
                            </div>
                            <div className="flex justify-end pt-6 space-x-3 border-t">
                                <button type="button" onClick={closeModal} className="bg-white border border-slate-300 py-2 px-4 rounded-lg" disabled={isLoading}>Скасувати</button>
