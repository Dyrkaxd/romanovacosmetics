...
import { EyeIcon, XMarkIcon, PlusIcon, TrashIcon, PencilIcon, DocumentTextIcon, PrinterIcon, FilterIcon, DownloadIcon } from '../components/Icons';
                ...
                {(activeOrderData.items || []).map((item, index) => (
                  <div key={item.id || `item-${index}`} className="grid grid-cols-1 sm:grid-cols-12 gap-2 mb-3 items-center p-3 border rounded-md bg-slate-50">
                    <div className="col-span-1 sm:col-span-12 md:col-span-5">
                      <label htmlFor={`product-${index}`} className="sr-only">Товар</label>
                       <select id={`product-${index}`} value={item.productId} onChange={(e) => handleItemChange(index, 'productIdSelect', e.target.value)} required aria-required="true" className="block w-full border-slate-300 rounded-md shadow-sm sm:text-sm p-2 focus:ring-indigo-500 focus:border-indigo-500">
