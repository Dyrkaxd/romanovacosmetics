...
const RecentOrdersList: FC<{ orders: ManagerDashboardData['recentOrders']; isLoading: boolean }> = ({ orders, isLoading }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-full">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Мої останні замовлення</h3>
            <div className="space-y-4">
                {isLoading ? (
                     [...Array(5)].map((_, i) => <div key={i} className="h-10 bg-slate-200 rounded-md animate-pulse"></div>)
                ) : orders.length === 0 ? (
                    <p className="text-sm text-slate-500 text-center py-8">У вас ще немає замовлень за цей період.</p>
                ) : (
                    orders.map(order => (
...
const TopProductsList: FC<{ products: ManagerDashboardData['topProducts']; isLoading: boolean }> = ({ products, isLoading }) => {
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-full">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Мої топ товари</h3>
             <div className="space-y-4">
                {isLoading ? (
                     [...Array(5)].map((_, i) => <div key={i} className="h-10 bg-slate-200 rounded-md animate-pulse"></div>)
                ) : products.length === 0 ? (
                    <p className="text-sm text-slate-500 text-center py-8">Немає даних про товари за цей період.</p>
                ) : (
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Вітаємо, {user?.name || 'Менеджер'}!</h2>
          <p className="text-slate-500">Ось огляд вашої діяльності.</p>
        </div>
