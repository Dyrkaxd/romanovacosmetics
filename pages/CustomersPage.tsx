...
const CustomersPage: React.FC = () => {
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try {
      ...
      // The backend function for customers now returns address as a nested object.
      // If it returned flat (address_street, etc.), you'd reconstruct here.
      // Since it's nested from the function, direct use is fine.
      setCustomers(data);
    } catch (err: any) {
  ...
  const handleSubmitCustomer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: `Failed to ${editingCustomer ? 'update' : 'add'} customer.`}));
        throw new Error(errorData.message || `HTTP error! status: ${response.status}`);
      }
      fetchCustomers(); 
      closeModal();
